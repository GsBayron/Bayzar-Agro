...
@Component({
  ...
  templateUrl: './listar-ingreso.html',
  styleUrl: './listar-ingreso.scss'
})
export class ListarIngreso implements OnInit {
  ...
  public dataSource = new MatTableDataSource<IIngreso>();
  ...
  public listar(): void {
    ...
    this.servicio.listar().subscribe({
      ...
      error: (err) => {
        ...
        this.mostrarMensaje(
          'Error',
          'No se pudieron cargar los ingresos registrados.',
          'error'
        );
      }
    });
  ...
  public actualizar(id: number | undefined): void {
    ...
    this.router.navigate([
      '/app/ingresos/actualizar',
      id
    ]);
  }

  public eliminar(item: IIngreso): void {
    ...
    const dialogRef = this.dialogo.open(Mensaje, {
      ...
      data: {
        titulo: 'Confirmación',
        mensaje: `¿Desea eliminar el ingreso "${item.descripcion}"?`,
        confirmacion: true
      }
    });
    ...
    dialogRef.afterClosed().subscribe((resultado: boolean) => {
      ...
      if (!resultado || !item.id_ingreso) {
        return;
      }

      this.servicio.eliminar(item.id_ingreso).subscribe({
        next: () => {
          this.mostrarMensaje(
            'Aviso',
