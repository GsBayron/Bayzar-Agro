...
@Component({
  ...
  templateUrl: './actualizar-actividad.html',
  styleUrl: './actualizar-actividad.scss'
})
export class ActualizarActividad implements OnInit {
  ...
  public consultar(): void {
    this.servicio.consultar(this.id_actividad).subscribe({
      next: (resp: IActividad) => {

        this.form.patchValue({
          id_cultivo: resp.id_cultivo as any,
          id_inventario: resp.id_inventario as any,

          tipo_actividad: resp.tipo_actividad,

          fecha_programada: resp.fecha_programada,
          fecha_realizacion: resp.fecha_realizacion ?? '',

          estado_actividad: resp.estado_actividad,
          prioridad: resp.prioridad,

          descripcion: resp.descripcion ?? '',

          cantidad_producto: resp.cantidad_producto ?? null,
          unidad_producto: resp.unidad_producto ?? '',
      ...
      error: (err) => {
        console.error(err);
        this.mensajeError('No se pudo consultar la actividad.');
      }
    });
  }
  ...
  public actualizar(): void {
    ...
    this.dialogo.open(Mensaje, {
      ...
      data: {
        titulo: 'Confirmación',
        mensaje: '¿Desea actualizar la actividad?',
        confirmacion: true
      }
      ...
      this.servicio.actualizar(
        this.id_actividad,
        payload as unknown as IActividad
      ).subscribe({
        next: () => {
